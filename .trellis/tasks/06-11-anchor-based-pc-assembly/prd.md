...
MVP 阶段的“真实装配”定义为结构语义正确，而不是 CAD / Blender 级逐毫米外观准确。也就是说，部件必须装到真实语义位置；没有对应 slot / anchor 时不能用假的 fallback 坐标掩盖问题；更换或移除单个部件不能重置其他实例；调试模式必须能暴露 slot、anchor、包围盒和冲突线索。贴图质量、不同机箱内部结构的毫米级校准、第三方模型外观相似度属于资产 intake 和人工校准阶段。

当用户选中的部件缺少可用 GLB / glTF、slot 或 anchor 时，产品仍可保留在产品页、配置清单和兼容性提示中，但不能生成假的 3D 安装实例。Builder 应显示“未安装 / 缺少模型或安装点”的冲突状态，3D 场景只渲染真实可装配的实例。

装配图严格依赖父级安装链：风扇和电源依赖机箱安装位，CPU / 显卡 / M.2 SSD 依赖主板 anchor，AIO 冷排依赖机箱冷排位。父级部件、slot 或 anchor 缺失时，子部件仍保留在配置清单和冲突提示里，但不生成孤儿 3D 实例，也不使用猜测坐标漂浮显示。

3D 装配状态以 installed instance 为单位，而不是以 catalog category 为单位。一个风扇商品可以生成 1 到 N 个风扇实例；一个 AIO 散热商品可以生成 pump 和 radiator 两个关联实例；未来多个 M.2 / 2.5 / 3.5 存储也应按单个安装实例管理。替换、移除、可见性和冲突状态都作用在 instance 上，配置清单和商品页仍可按商品 / category 展示。

产品选择层允许用户选中不兼容部件，用于比较和解释原因；3D 装配层只渲染结构可安装的实例。显卡长度、显卡槽宽、电源规格 / 长度、水冷冷排尺寸、主板规格、风扇尺寸等冲突必须绑定到具体 installed instance candidate 或 mount slot，并在配置清单 / 兼容性面板说明原因，不能在 3D 画面中假装可安装。

预设 setup 和用户上传 setup 保存产品选择和 installed instance overrides，而不是保存最终 3D 坐标。例如 setup 可以保存选中的机箱、主板、显卡、电源和风扇商品，以及某个风扇实例安装到哪个 `slotId`、某个实例是否被移除或替换。调试校准后的 anchor / slot 属于模型资产配置，不属于 setup。重新加载 setup 时，装配引擎应根据当前资产 anchor、slot 和装配规则重新计算位置。

资产校准写回仅属于本地开发 / 资产管理员流程。调试模式可以导出 anchor / slot patch，或在本地开发环境写回 `local-model-assets.json`；线上生产环境不允许普通用户直接修改全局模型资产。用户上传 setup 只能包含产品选择和 installed instance overrides。校准结果必须经过资产管理员 / HITL 审核后，才能进入正式资产库。
...
- Replacement, removal, visibility and conflict state should operate on installed instances rather than whole catalog categories.
- Multi-instance products such as fan packs, AIO coolers and future storage groups should expand into one or more installed instances while preserving product-level catalog display.
...
- Parts without usable model assets or resolved slots / anchors should remain selectable in commerce/configuration surfaces but must not create fake 3D scene instances.
- Child parts with missing parent mount targets should report an uninstalled dependency conflict instead of rendering as orphan fallback geometry.
...
- Product selection should not be blocked by compatibility conflicts, but 3D installation should render only structurally installable instances.
- Compatibility conflicts should reference an installed instance candidate or mount slot whenever possible.
...
- Preset and uploaded setups should store product selections and installed-instance overrides, not final scene coordinates.
- Anchor and slot calibration belongs to model asset metadata and should not be serialized into setup data.
- Asset calibration write-back should be limited to local development or asset-admin workflows, not ordinary production users.
- Production setup uploads must not mutate global model asset metadata.
